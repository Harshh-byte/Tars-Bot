...
export default function Hero() {
  ...
  const phrases = [
    "Certified professional instigator.",
    "Emotionally unavailable. Technically online.",
    "Generating violence in lowercase.",
    "Here to help. Mostly to roast.",
  ];
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      gsap.set(".hero-title-text", { y: 0, opacity: 1 });
      gsap.set(".hero-description-container", { y: 0, opacity: 1 });
      gsap.set(".hero-action-btn", { scale: 1, opacity: 1 });
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-6 w-full relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
        <div className="lg:col-span-7 flex flex-col items-center text-center lg:items-start lg:text-left">
          <h1 className="hero-title-text font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[0.95] tracking-tight uppercase mb-6 text-ds-text">
            Meet <br />
            <span
              className={`text-gradient text-ds-blurple ${isDecrypted ? "shiny-text animate-shine" : ""}`}
            >
          ...
          <div className="hero-description-container flex items-center justify-center lg:justify-start mb-8 min-h-[4em] text-lg sm:text-xl font-mono text-ds-muted leading-relaxed max-w-xl">
            <span className="font-mono text-ds-text">
              {typewriterText}
              <span className="animate-[pulse_1s_infinite] font-black text-ds-text ml-0.5">
                |
              </span>
            </span>
          </div>
        ...
        <div className="lg:col-span-5 flex items-center justify-center relative w-full overflow-hidden">
          <div
            ref={heroImageRef}
            className="floating-avatar-container relative flex items-center justify-center p-6 sm:p-12 w-full max-w-[320px] sm:max-w-none mx-auto"
          >
            ...
            <CircularText
              text="• Savage Replies • Smart Automation • Community Management "
              radius={120}
              fontSize="9px"
